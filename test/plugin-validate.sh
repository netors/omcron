#!/usr/bin/env bash
# VENDORED from /usr/share/omarchy/bin/omarchy-plugin-validate
# Omarchy 4.0.0.alpha (package 4.0.1-1)
# Vendored rather than reimplemented so CI failures read exactly like the ones
# users get, and so re-syncing after an Omarchy release is a diff, not a rewrite.
# Re-check this file on each Omarchy upgrade.
